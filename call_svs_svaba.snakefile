...
def expected_svaba_result_sv_files():
	...
	for base in illumina_read_bases:
		result.append(os.path.join(raw_sv_calls_dir, "raw", base + "_indel_svaba.vcf"))
		result.append(os.path.join(raw_sv_calls_dir, "raw", base + "_sv_svaba.vcf"))
	result = list(result)
	return result
...
rule copy_svaba_vcf_to_aggregate_dir:
	...
	output: 
		indel=os.path.join(raw_sv_calls_dir, "raw", "{base}_indel_svaba.vcf"),
		sv=os.path.join(raw_sv_calls_dir, "raw", "{base}_sv_svaba.vcf")
